...
class MixGate(nn.Module):
    ...
    def forward(self, feat1, feat2, sim, freqs, total_freqs, ctx=None):
        ...
        freqs = freqs.to(torch.float32) #change
...
class SurrogateDistillClassifierLayer(nn.Module):
    def __init__(self, input_dim:int, output_dim:int, mu:float, lam:float, att_loss:bool=True, nheads:int=2, fusion_layer:int=5, token_freq_tensor:Optional[torch.Tensor]=None, type_token_tensor:Optional[Dict]=None):
        super().__init__()
        self.nheads = nheads
    ...
    def generate_substitute2(self, r, r_ctx=None, freqs=None, total_freqs=None, training=True, bert_model=None):
        ...
        if self.use_type_token:
            ...
            self.type_weight = self.type_weight.to(torch.float32) #change
