...
class Worker(object):
    ...
    def __init__(self, opts, make_log_dir=True):
        ...
        self.gpu = opts.gpu
        self.gpu_ids = [0] #[int(t.strip()) for t in opts.gpu.strip().split(",")]
    ...
    def eval(self, model, loader, eval_metric, eval_args):
        ...
        '''import pandas as pd
        paraphrases = {
            'entity': ['Shawshank Redemption','Shawshank Redemption'],
            'entity_type': ['film','film'],
            'original_sentence': ["'The Shawshank Redemption' is a story of hope and perseverance.",
                                  "'The Shawshank Redemption' is a story of hope and perseverance."],
            'paraphrased_sentence': ["Andy and Red's friendship shines in 'The Shawshank Redemption.'", 
                                     "'The Shawshank Redemption' ends with Andy's triumphant escape."]
        }
        paraphrases_df = pd.DataFrame.from_dict(paraphrases)
        tokenizer = AutoTokenizer.from_pretrained('roberta-large', add_prefix_space=True)
        # Step 4: Function to mask the entity in the fixed sentence template
        def mask_entity_in_sentence(sentence, entity):
            entity_tokens = tokenizer.tokenize(entity)
            masked_entity = ' '.join(['<mask>'] * len(entity_tokens))

            # Convert both sentence and entity to lowercase for case-insensitive replacement
            lower_sentence = sentence.lower()
            lower_entity = entity.lower()

            if lower_entity in lower_sentence:
                masked_sentence = lower_sentence.replace(lower_entity, masked_entity, 1)
            else:
                print(f"Entity '{entity}' not found in sentence: {sentence}")
                masked_sentence = sentence
            return masked_sentence, entity_tokens

        # Step 5: Function to calculate the probability of the true entity
        def calculate_probability(masked_sentence, true_tokens):
            probabilities = []

            for i, true_token in enumerate(true_tokens):
                inputs = tokenizer(masked_sentence, return_tensors='pt')
                print("MASKED SENT : ", masked_sentence)
                print("INPUTS : ",inputs)
                print("MASKED ID : ", tokenizer.mask_token_id)
                print("MASKED TOKEN : ", tokenizer.mask_token)
                mask_token_index = torch.where(inputs.input_ids == tokenizer.mask_token_id)[1]

                if len(mask_token_index) == 0:
                    print(f"Masked token not found in the sentence: {masked_sentence}")
                    return 0  # Return zero probability if mask token not found

                with torch.no_grad():
                    token_logits = model(**inputs).logits
                LOGGER.info("*"*30)
                LOGGER.info(token_logits)
                LOGGER.info(token_logits.shape)
                mask_token_logits = token_logits[0, mask_token_index, :]
                mask_token_probs = torch.softmax(mask_token_logits, dim=1)

                token_id = tokenizer.convert_tokens_to_ids(true_token)
                if token_id >= mask_token_probs.shape[1]:
                    print(f"Token ID {token_id} out of bounds for the probabilities array.")
                    return 0  # Return zero probability if token ID is out of bounds

                token_probability = mask_token_probs[0, token_id].item()
                probabilities.append(token_probability)

                # Update the masked sentence for the next word
                if i < len(true_tokens) - 1:
                    next_masked_sentence = masked_sentence.replace('[MASK]', true_token, 1)
                    masked_sentence = next_masked_sentence

            entity_probability = np.prod(probabilities)
            return entity_probability

        # Step 6: Calculate probabilities for the paraphrased sentences
        results = []

        for _, row in tqdm(paraphrases_df.iterrows(), total=paraphrases_df.shape[0], desc="Calculating probabilities"):
            entity = row['entity']
            entity_type = row['entity_type']
            original_sentence = row['original_sentence']
            paraphrased_sentence = row['paraphrased_sentence']
            masked_sentence, true_tokens = mask_entity_in_sentence(paraphrased_sentence, entity)
            probability = calculate_probability(masked_sentence, true_tokens)
            results.append({
                'entity': entity,
        ...
        results_df.to_csv('entity_probabilities_with_paraphrases.csv', index=False)'''
        ##############
